/**
 * JSON schema for the structured discovery extraction. Used as Gemini's
 * `responseSchema` so the model is forced to return JSON matching this shape
 * (no parsing free-form prose, no markdown code fences).
 *
 * Mirrors `DiscoveryInput` in `lib/types/discovery.ts` — keep them in sync.
 */
  ...
  properties: {
    prospect: {
      ...
      properties: {
        ...
        industry: {
          type: 'string',
          description: "Industry vertical, e.g. 'Fashion & Apparel'.",
        },
        ...
        currency: {
          ...
          description:
            'ISO 4217 code derived from any currency symbol or name mentioned.',
        },
      },
    },
    program: {
    ...
    pointsIssued: {
      type: 'number',
      description: 'Points issued during the period above.',
    },
    pointsRedeemed: {
      type: 'number',
      description: 'Points redeemed during the period above.',
    },
    outstandingPoints: {
      type: 'number',
    ...
    pointValue: {
      type: 'number',
      description:
        'Value of one point at redemption, in the local currency (e.g. ₹0.10 → 0.1).',
    },
...
1. Respond with JSON matching the provided schema. No prose, no markdown code fences.
